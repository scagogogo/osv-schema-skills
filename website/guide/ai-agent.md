...
```mermaid
flowchart TD
  IN["User input"] --> D{"Intent classification"}
  D -->|"read/inspect"| P["osv parse"]
  D -->|"compliance"| V["osv validate"]
  D -->|"narrow down"| F["osv filter"]
  D -->|"extract field"| Q["osv query"]
  P --> OUT["Summarized report"]
  V --> OUT
  F --> OUT
  Q --> OUT
```
